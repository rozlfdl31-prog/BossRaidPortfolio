...
* **Boss Lunge Root Motion Relay Rule**: Lunge 이동은 `rushPhaseRatio/MoveRaw` 수동 전진이 아니라, Animator `OnAnimatorMove`의 `deltaPosition`을 `BossController.ApplyLungeRootMotion`으로 전달해 부모 루트를 이동시킨다. 활성화 구간은 `SetLungeRootMotionEnabled(true/false)`로 제한하며, 준비 구간에서는 Y를 차단하고 Launch 이후에만 Y를 허용한다. 릴레이는 Lunge 시작 시 `Visual` 로컬 기준 포즈를 캐시하고 `OnAnimatorMove`/종료 시 복원해 부모 루트와 자식 비주얼 좌표가 분리되지 않도록 유지한다. Lunge 타이밍은 히트박스 종료(`normalizedTime 0.8`)와 상태 종료(`normalizedTime 1.0`)를 분리해 운영한다. Attack2 Landing 디버그가 켜져 있으면 `RootMotionMove`/`RootMotionRelayProbe`는 Gravity 로그와 별도 샘플러를 사용해 누락 없이 추적한다.
