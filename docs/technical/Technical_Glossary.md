...
* **Lobby Active Session (Lobby active 세션)**: Host와 Client가 같은 `LobbyPanel`에서 기다리는 세션. Host는 `15s` heartbeat를 유지하고, both peers는 optional Lobby Events + `2s` poll fallback으로 latest lobby state를 맞춘다. Host는 lobby `SessionState` metadata를 `Waiting/Ready`로 관리하고, `2/2 connected + NGO stable + 2초` 뒤에만 `Start`를 열 수 있다.
* **Gameplay Start Session (gameplay start 세션)**: Host가 `StartGameplayAsync()`로 `LobbyActive -> StartingGameplay` 전환을 열고, lobby `SessionState=Starting`을 publish한 뒤 NGO synchronized scene load로 both peers를 current fast multiplayer target(`Assets/Scenes/mutiplayer/GamePlayScene_Verify.unity`)에 넣는 세션. success 기준은 "same scene synchronized enter"이고, spawn/gameplay ownership은 후속 세션으로 남긴다. 2026-03-19 manual smoke에서 same lobby/session + same gameplay scene enter는 확인됐고, player model selection / boss gameplay shared state는 이 세션의 범위 밖이다.
* **Strict Cleanup Session (strict cleanup 세션)**: `Back`, `Cancel`, wrong key 이후 fatal fail, disconnect, host exit 같은 종료 경로를 항상 같은 close order로 처리하는 세션. current runtime에서는 `MultiplayerSessionService`가 `Closing` busy guard, lobby delete/leave, NGO shutdown, cached state clear, title return까지 owner로 처리한다.
* **StartingGameplay State**: `MultiplayerSessionService`가 gameplay scene sync 진행 중임을 나타내는 runtime state. lobby status text는 `Starting game...`로 고정되고, timeout / disconnect / scene load fail은 strict cleanup으로 떨어진다.
* **SessionState Metadata**: Host가 Lobby public data에 쓰는 진행 상태 값. current multiplayer flow에서는 `Waiting`, `Ready`, `Starting` 세 값을 사용해 client UI status text와 host `Start` availability를 같은 source로 맞춘다.
* **Session Version Guard (세션 버전 가드)**: `MultiplayerSessionService`가 create/join/cleanup 시작마다 version counter를 올리고, lobby refresh / heartbeat / host `SessionState` update / optional lobby event callback이 자신이 시작된 version과 current version이 다르면 결과를 버리는 보호 장치. strict cleanup 뒤 old async result가 새 session snapshot이나 in-flight flag를 덮지 못하게 막는다.
* **Multiplayer Verify Scene (멀티플레이 검증 씬)**: fast multiplayer smoke test 전용 lightweight gameplay scene. 현재 경로는 `Assets/Scenes/mutiplayer/GamePlayScene_Verify.unity`이며, `Ground`, `Wall`, `Systems`, `ResultTestTrigger`, shared `Canvas`만 유지해 Host/Client/Lobby/gameplay start/result flow를 빠르게 확인한다. current manual smoke 기준으로는 same-scene enter acceptance를 검증하는 용도이고, full shared gameplay state까지 보장하지는 않는다.
* **Fast Multiplayer Verify Build Runner (빠른 멀티플레이 검증 빌드 러너)**: `Assets/Editor/FastMultiplayerVerifyBuildRunner.cs`에 있는 editor build helper. `Tools/Build/Fast Multiplayer Verify Build` 메뉴에서 `TitleScene + GamePlayScene_Verify`만 Windows build로 만들고, build 동안 `Assets/Map/Beautify/URP/Runtime/Resources`를 `Resources~`로 잠시 제외해 Beautify shader compile 부담을 줄인 뒤 자동 복구한다.
...
* **Partner HUD Visibility Gate (파트너 HUD 표시 게이트)**: `CombatHUDController`가 `PartnerHUD_Panel`을 기본 hidden으로 유지하고, `PlayerController.InitializeCombatHUD()`가 `MultiplayerSessionService.HasActiveSession`이 true일 때만 이를 표시하는 규칙. duplicated multiplayer title scene의 `Solo Play`도 duplicated multiplayer verify gameplay scene path를 사용할 수 있으므로 scene path는 truth source로 쓰지 않는다.
...
* **LobbyPanel**: Host/Client가 공통으로 보는 멀티플레이 대기 패널. room title, join code, connected players, waiting text, host-only `Start`를 표시한다. runtime 기준 `Start`는 Host에게만 보이고, service snapshot의 `CanStart`가 `true`일 때만 활성화된다. local `StartingGameplay` 또는 lobby `SessionState=Starting`이면 status text는 `Starting game...`으로 묶는다.
...
* **Start Unlock Gate**: Host `Start` 버튼이 `2/2 connected` 직후 즉시 열리지 않고, `MultiplayerSessionService`가 `NGO stable + 2초 유지`를 확인한 뒤 `CanStart = true`를 내보내는 게이트.
* **MultiplayerTitleSceneDriver**: duplicated multiplayer title scene에서만 runtime에 붙는 wrapper 컴포넌트. Host `Create Room`, Client `Join`, lobby `Cancel/Back`, Host `Start`를 `MultiplayerSessionService`로 라우팅하고, 진행 중 버튼 잠금 / `Connecting...` / `Starting...` 표시 / wrong key popup 유지 / fatal fail title 복귀를 담당한다. Session 7부터는 `StateChanged(Closing)`도 구독해 cleanup 시작 직후 local `_isBusy`를 올리고 `Closing...` label로 새 action을 막는다. `Solo Play`만 duplicated multiplayer verify scene path를 직접 로드하고, Host `Start`는 local scene load를 직접 실행하지 않는다.
...
* **MultiplayerSessionService**: 멀티플레이 session owner. Host create / Client join / Lobby active / gameplay start / strict cleanup 동안 Relay, Lobby, NGO 상태를 한 곳에서 관리하고, UI에는 snapshot event로 room title / join code / player count / status text를 전달한다. Session 7 기준으로 host heartbeat, `2s` poll fallback, host-owned `SessionState(Waiting/Ready/Starting)` sync, `CanStart` 계산, `StartGameplayAsync()` host guard, NGO scene sync completion gate, `15s` timeout, client disconnect / lobby gone fatal cleanup mapping, `Closing` busy guard, session-version stale async invalidation까지 포함한다. current default gameplay target은 lightweight verify scene이다.
* **Strict Close**: 멀티플레이 흐름 중 create/join/start 실패 또는 `Back/Cancel` 발생 시 세션을 닫고 `TitleScene`으로 돌아가는 정리 규칙. 현재 Host create / Client join / Lobby active / gameplay start path에서는 lobby delete(or remove player), heartbeat stop, NGO shutdown, cached state clear까지 같은 owner가 정리하며, host cancel / client local disconnect / lobby gone failure / mid-start scene sync fail도 이 cleanup owner로 묶인다. cleanup 중에는 new `Create/Join/Start`보다 cleanup complete가 우선이고, old async callback result는 session-version guard로 폐기된다.
